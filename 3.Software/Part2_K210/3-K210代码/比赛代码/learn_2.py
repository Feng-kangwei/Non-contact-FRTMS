        ...
        for i in code:
            ...
            if max_score > ACCURACY:
                ...
                if(read_data):
                    ...
                    print(index_State)
                #####################################
                print("识别结果为：%s"%Face_Recognition)
                if(Face_Recognition[0]=='F'):
                    uart2.write("t0.txt=\"FKW\"")
                    uart2.write(byte16)
                elif(Face_Recognition[0]=='W'):
                    uart2.write("t0.txt=\"WXY\"")
                    uart2.write(byte16)
                elif(Face_Recognition[0]=='Z'):
                    uart2.write("t0.txt=\"ZZL\"")
                    uart2.write(byte16)
                elif(Face_Recognition[0]=='U'):
                    uart2.write("t0.txt=\"ADDER1\"")
                    uart2.write(byte16)
                else:
                    uart2.write("t0.txt=\"NO FACE\"")
                    uart2.write(byte16)

            else:
                a = img.draw_string(i.x(), i.y(), ("X :%2.1f" % (
                    max_score)), color=(255, 0, 0), scale=2)
                uart2.write("t0.txt=\"NO KNOWN FACE\"")
                uart2.write(byte16)
            if start_processing:
                record_ftr = feature
